package fr.maxlego08.core.utils.gson;
...
public class Persist {
	...
	public <T> T loadOrSaveDefault(T def, Class<T> clazz, File file)
	{
		if ( ! file.exists())
		{
			p.log("Creating default: "+file, LogType.INFO);
			this.save(def, file);
			return def;
		}
		...
		if (loaded == null)
		{
			p.log("Using default as I failed to load: "+file, LogType.WARNING);
			...
			p.log("Backing up copy of bad file to: "+backup, LogType.WARNING);
	...
	{
		return DiscUtil.writeCatch(file, p.getGson().toJson(instance));
	}
	...
	public <T> T load(Class<T> clazz, File file)
	{
		...
		{	// output the error message rather than full stack trace; error parsing the file, most likely
			p.log(ex.getMessage(), LogType.ERROR);
		}
