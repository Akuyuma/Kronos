...
public class FactionsChatListener implements Listener
{
	...
	@EventHandler
	public void FactionMessage(AsyncPlayerChatEvent e){
		String format = e.getFormat().toString();
		FPlayer f = FPlayers.i.get(e.getPlayer());
		if (format.contains("<classement>")){
			format = format.replace("<classement>", getStringClassement(f.getFaction().getClassementPosition()));
		}
		e.setFormat(format);
		
	}

	public String getStringClassement(int classement){
		if (classement == 1){return "§4"+classement;}
		if (classement == 2){return "§c"+classement;}
		if (classement == 3){return "§6"+classement;}
		return "§e"+classement;
	}
