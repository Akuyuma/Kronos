...
public class FactionsEntityListener implements Listener
{
	...
	public void onEntityDeath(EntityDeathEvent event)
	{
		...
		if (faction.isWarZone())
		{
			...
			{
				powerLossEvent.setMessage("<b>Le monde dans lequel vous êtes a une perte de puissance normalement désactivée, mais vous avez quand même perdu le pouvoir puisque vous étiez dans une zone de guerre.\n<i>Votre power est maintenant <h>%d / %d");
			}
		}
		else if (faction.isNone() && !Conf.wildernessPowerLoss && !Conf.worldsNoWildernessProtection.contains(player.getWorld().getName()))
		{
			powerLossEvent.setMessage("<i>Vous n'avez perdu aucun power depuis que vous étiez dans le désert..");
			powerLossEvent.setCancelled(true);
		}
		else if (Conf.worldsNoPowerLoss.contains(player.getWorld().getName()))
		{
			powerLossEvent.setMessage("<i>Vous n'avez perdu aucun power en raison du monde dans lequel vous êtes mort.");
			powerLossEvent.setCancelled(true);
		}
		else if (Conf.peacefulMembersDisablePowerLoss && fplayer.hasFaction() && fplayer.getFaction().isPeaceful())
		{
			powerLossEvent.setMessage("<i>Vous n'avez perdu aucun power puisque vous êtes dans une faction pacifique.");
			powerLossEvent.setCancelled(true);
		}
		else {
	...
	public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify)
	{
		...
		{
			if (notify) attacker.msg("§6» <i>Vous ne pouvez pas blesser les autres joueurs pour " + Conf.noPVPDamageToOthersForXSecondsAfterLogin + " secondes après la connexion.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("§6» <i>Vous ne pouvez pas blesser les autres joueurs pendant que vous êtes dans "+(locFaction.isSafeZone() ? "a SafeZone." : "peaceful territory."));
			return false;
		}
		...
		{
			if (notify) attacker.msg("§6» <i>Vous ne pouvez pas blesser les autres joueurs jusqu'à ce que vous rejoignez une faction.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("§6» <i>Vous ne pouvez pas blesser les joueurs pendant que vous êtes dans une faction pacifique.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("§6» <i>Vous ne pouvez blesser les factions neutres. Déclarez-les comme un ennemi.");
			return false;
		}
		...
		if (ownTerritory && relation.isNeutral())
		{
			if (notify)
			{
				attacker.msg("§6» <i>Vous ne pouvez pas blesser %s<i> dans son territoire sauf si vous le déclarez ennemie.", defender.describeTo(attacker));
		...
		if (damage > 0.0 && ownTerritory && Conf.territoryShieldFactor > 0)
		{
			...
			if (notify)
			{
				...
				defender.msg("§6» <i>Dommages ennemis réduits de <rose>%s<i>.", perc);
			}
		}
