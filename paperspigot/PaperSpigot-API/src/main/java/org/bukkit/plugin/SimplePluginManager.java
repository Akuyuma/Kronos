...
public final class SimplePluginManager implements PluginManager {
    ...
    public void registerEvents(Listener listener, Plugin plugin, String prefix) {
    	if (!plugin.isEnabled()) {
    		throw new IllegalPluginAccessException("Plugin attempted to register " + listener + " while not enabled");
    	}
    	
    	Map<Class<? extends Event>, Set<RegisteredListener>> map = plugin.getPluginLoader().createRegisteredListeners(listener, plugin);
    	
    	for (Map.Entry<Class<? extends Event>, Set<RegisteredListener>> entry : map.entrySet()) {
    		getEventListeners(getRegistrationClass(entry.getKey())).registerAll(entry.getValue());
    	}
    	Bukkit.getConsoleSender().sendMessage(prefix + " §7Register listener '§2"+listener.getClass().getName()+"§7', §e" + map.size()+" §7event register !");
