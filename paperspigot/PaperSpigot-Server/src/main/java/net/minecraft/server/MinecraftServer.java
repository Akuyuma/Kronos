...
public abstract class MinecraftServer implements ICommandListener, Runnable, IMojangStatistics {
    ...
    protected void g() {
        ...
        for (int m = 0; m < this.worlds.size(); ++m) {
            ...
            MinecraftServer.i.info("Chargement des regions du monde " + m + " (Seed: " + worldserver.getSeed() + ")");
            ...
            for (int k = -192; k <= 192 && this.isRunning(); k += 16) {
                for (int l = -192; l <= 192 && this.isRunning(); l += 16) {
                    ...
                    if (i1 - j > 1000L) {
                        this.a_("Génération des régions", i * 100 / 625);
                        j = i1;
                    }
    ...
    protected void saveChunks(boolean flag) throws ExceptionWorldConflict { // CraftBukkit - added throws
        if (!this.M) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                if (worldserver != null) {
                    if (!flag) {
                        MinecraftServer.i.info("Sauvegarde des chunks de la map \'" + worldserver.getWorldData().getName() + "\'/" + worldserver.worldProvider.getName());
                    }
    ...
    public void stop() throws ExceptionWorldConflict { // CraftBukkit - added throws
        if (!this.M) {
            ...
            if (this.u != null) {
                i.info("Sauvegarde des joueurs");
                this.u.savePlayers();
                this.u.u();
            }

            if (this.worldServer != null) {
                i.info("Sauvegarde des mondes");
