...
public class PlayerConnection implements PacketPlayInListener {
    ...
    private Map<Player, Location> playerWalks = new HashMap<Player, Location>();
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if (!playerWalks.containsKey(player)){
            	playerWalks.put(player, to);
            }else{
            	if (playerWalks.get(player).getWorld().getName().equals(to.getWorld().getName()) && playerWalks.get(player).distance(to) >= 1){
            		
            		playerWalks.put(player, to);
                   	/* On call le PlayerWalkEvent */
                    
                	PlayerWalkEvent event = new PlayerWalkEvent(player, from, to);
                	this.server.getPluginManager().callEvent(event);
                    if (event.isCancelled()) {
                        this.player.playerConnection.sendPacket(new PacketPlayOutPosition(playerWalks.get(player).getX(), playerWalks.get(player).getY() + 1.6200000047683716D, playerWalks.get(player).getZ(), playerWalks.get(player).getYaw(), playerWalks.get(player).getPitch(), false));
                        return;
                    }
